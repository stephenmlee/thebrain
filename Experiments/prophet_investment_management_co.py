...
def balance():
    return money if money > 0 else shares * ftse100[len(ftse100) - 1]


if __name__ == "__main__":
    starting_genome = decode('Experiments/CryogenicStorage/prophet_fund_manager.json')

    ftse100_data = decode('Experiments/Vault/ftse100-2014-15.json')
    ftse100 = []
    volume = []
    ...
    brain = NeuralNetwork.expressed_from(starting_genome)
    ...
    for index, quote in enumerate(ftse100):
        ...
        print "%.4f, %.4f, %.4f " % (ftse100_t1[index], prediction, balance())
